import { CreateIdentityPoolOutput } from './CreateIdentityPoolOutput';
import { DeleteIdentitiesOutput } from './DeleteIdentitiesOutput';
import { DeleteIdentityPoolOutput } from './DeleteIdentityPoolOutput';
import { DescribeIdentityOutput } from './DescribeIdentityOutput';
import { DescribeIdentityPoolOutput } from './DescribeIdentityPoolOutput';
import { GetCredentialsForIdentityOutput } from './GetCredentialsForIdentityOutput';
import { GetIdOutput } from './GetIdOutput';
import { GetIdentityPoolRolesOutput } from './GetIdentityPoolRolesOutput';
import { GetOpenIdTokenOutput } from './GetOpenIdTokenOutput';
import { GetOpenIdTokenForDeveloperIdentityOutput } from './GetOpenIdTokenForDeveloperIdentityOutput';
import { ListIdentitiesOutput } from './ListIdentitiesOutput';
import { ListIdentityPoolsOutput } from './ListIdentityPoolsOutput';
import { LookupDeveloperIdentityOutput } from './LookupDeveloperIdentityOutput';
import { MergeDeveloperIdentitiesOutput } from './MergeDeveloperIdentitiesOutput';
import { SetIdentityPoolRolesOutput } from './SetIdentityPoolRolesOutput';
import { UnlinkDeveloperIdentityOutput } from './UnlinkDeveloperIdentityOutput';
import { UnlinkIdentityOutput } from './UnlinkIdentityOutput';
import { UpdateIdentityPoolOutput } from './UpdateIdentityPoolOutput';
export declare type OutputTypesUnion = CreateIdentityPoolOutput | DeleteIdentitiesOutput | DeleteIdentityPoolOutput | DescribeIdentityOutput | DescribeIdentityPoolOutput | GetCredentialsForIdentityOutput | GetIdOutput | GetIdentityPoolRolesOutput | GetOpenIdTokenOutput | GetOpenIdTokenForDeveloperIdentityOutput | ListIdentitiesOutput | ListIdentityPoolsOutput | LookupDeveloperIdentityOutput | MergeDeveloperIdentitiesOutput | SetIdentityPoolRolesOutput | UnlinkDeveloperIdentityOutput | UnlinkIdentityOutput | UpdateIdentityPoolOutput;
